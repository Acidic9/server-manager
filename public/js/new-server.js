...
function submitNewServerForm(event) {
        ...
        if (!resp.success) {
            switch (resp.error) {
                case "missing-deps":
                    ...
                    li += "</ul>";
                    alertify.okBtn("Install as root").confirm("The remote host is missing the following dependencies:" + li, function () {
                        alertify.reset().prompt("Enter <b>root</b> password for remote host", function (password) {
                            $("#submit-new-server-form").addClass("is-loading");
                            $("#new-server-form").attr("disabled", 1);
...
function refreshMachines() {
    ...
    $.get("/machine/list", function (resp) {
        if (!resp.success) {
            showError(resp.error);
            return;
        }
