var machines: {
	[key: string]: { [key: string]: Server }
} = {};
var selectedServers: number[][] = []; // selectedServers[[machineID, serverID], ...]
...
function onServerCheckboxChange() {
	...
	if (cbox.is(":checked")) {
		selectedServers.pushIfNotExist(([machineID, serverID] as number[]));
...
function setServerRowLoading(tr, loading) {
	//if (typeof tr == 'object')
	// 	tr = $("#server-"+tr.serverID+'-'+tr.machineID);
